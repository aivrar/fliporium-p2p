...
const peerPort = 41642

// genTLSConfig produces a fresh in-memory TLS config with a self-signed cert.
// Both server and client sides use InsecureSkipVerify because we trust the
// Tailscale layer for peer authentication.
func genTLSConfig(hostname string) (*tls.Config, error) {
...
// HubEvent is a single async event to surface to the REPL.
...
func (h *Hub) emit(ev HubEvent) {
	...
	case h.Events <- ev:
	default:
		// drop if UI is not consuming fast enough
	}
}
...
func (h *Hub) Dial(ctx context.Context, dial func(ctx context.Context, network, addr string) (net.Conn, error), tlsCfg *tls.Config, target, selfName string) error {
	addr := fmt.Sprintf("%s:%d", target, peerPort)
