// Phase 3 entry point: a tsnet-backed Fliporium peer with terminal chat.
//
// Configuration (env vars):
//
//	FLIPORIUM_AUTHKEY   — Headscale pre-auth key (required on first run only;
//	                      identity persists in the data dir afterwards).
//	FLIPORIUM_HOSTNAME  — node hostname (default "fliporium").
...
func main() {
	...
	hostname := env("FLIPORIUM_HOSTNAME", "fliporium")
	...
	// Quiet logger for tsnet so the REPL stays readable. Verbose logs are still
	// available — wrap to file later if needed.
	...
	tlsCfg, err := genTLSConfig(hostname)
	if err != nil {
	...
	if headless {
		...
		// Don't close replDone, so the select below waits on sigC only.
	} else {
		go func() {
...
func displayEvents(events <-chan HubEvent) {
	for ev := range events {
...
func runREPL(ctx context.Context, srv *tsnet.Server, hub *Hub, tlsCfg *tls.Config, selfName string) {
...
func handleSay(hub *Hub, rest string) {
	names := hub.Names()
	if len(names) == 0 {
...
func handleDisconnect(hub *Hub, rest string) {
	target := rest
	if target == "" {
		names := hub.Names()
		if len(names) != 1 {
	...
	_ = c.WriteFrame(TypeBye, Bye{Reason: "user disconnected"})
	c.Close()
}
